...
export class TieredFee implements Fee {
    ...
    calculateFor(amount: Money): Money {
        ...
        for (let tier of this.tiers) {
            ...
            if (tier.tierLimit !== null) {
                tierAmount = tier.tierLimit.isLesserThan(amount) ? tier.tierLimit : amount;
            } else {
                tierAmount = amount;
            }
    ...
    public getExtendedDescription(): string[] {
        ...
        return this.tiers.map((tier: Tier): string => {
            ...
            return 't/m ' + numberFormatter.formatMoney(tier.tierLimit, 0) + ': ' + tier.fee.describe();
        });
    }
}

export class Tier {
    constructor(public tierLimit: Money, public fee: Fee) {
    }
}
