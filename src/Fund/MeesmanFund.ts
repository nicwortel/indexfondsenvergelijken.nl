...
export class MeesmanFund extends MutualFund {
    constructor(
        ...
        super(name, identifier, isin, logo, totalExpenseRatio, internalTransactionCosts, dividendLeak, index, kiid, factsheet, esgExclusions);
    }

    getTotalExpenseRatio(totalInvested?: Money): Percentage {
        const discountThreshold: Money = new Money(1000000, 'EUR');

        if (totalInvested !== undefined && totalInvested.isGreaterThanOrEqual(discountThreshold)) {
    ...
    getTerComment(): string {
        const numberFormatter = new Intl.NumberFormat('nl-nl', {minimumFractionDigits: 2});

        return numberFormatter.format(this.reducedTotalExpenseRatio.getPercentage()) + '% bij een belegd vermogen van € 1 miljoen of meer';
    }
}
