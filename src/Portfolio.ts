...
export class Portfolio {
  constructor(
    public assets: { allocation: Percentage; fund: Fund }[],
  ...
  public getYearlyCosts(expectedDividend: Percentage): Percentage {
    return this.assets.reduce((sum: Percentage, asset) => {
      ...
      const fundCosts = fund
        .getTotalExpenseRatio()
  ...
  public getTotalExpenseRatio(): Percentage {
    return this.assets.reduce(
      (sum: Percentage, asset) =>
        sum.add(asset.allocation.multiply(asset.fund.getTotalExpenseRatio())),
      new Percentage(0)
    );
  }
  ...
  public getDividendLeak(): Percentage {
    return this.assets.reduce(
      (sum: Percentage, asset) =>
        sum.add(asset.allocation.multiply(asset.fund.getDividendLeak())),
      new Percentage(0)
    );
  }
